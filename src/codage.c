...
static Huffman *initHuffman(int *tab, int len){
    int cmpt = 0;
    int i = 0;
    ...
    if (!huffman->array) { free(huffman); return NULL; }
...
static Huffman *huffmanTree(FILE* fptr){
...
static char **construitCode(Huffman *huffman){
    int i = 0, j = 0;
    char **traduc = NULL;
...
void codeHuffmanX8(FILE *in, FileBit *out){
    ...
    */
    ...
    for (i = 0; i < padding; ++i) { fEcrireBit(out, 0); }
    destroy_huffman(huffman);
...
int codage_fichier(char *nom_fichier){
    ...
    if (!fptr) { fprintf(stderr, "%s probleme de fichier\n", nom_fichier); return 1; }
...
static void affiche_code(char **traduc){
    unsigned char c = 0;
    for (c = 0; c < NBLETTRES / 2; ++c)
        if ( longueur( *(traduc + c) ) > 0 && est_alpha(c) )
            printf("%c: %s \n", c, traduc[c]);
}
*/
